...
class lookUpTable {
    ...
    getCountry(trigram){
        log.trace("getCountry for trigram ",trigram);
        var country = this._lookUpExcFile[trigram]["Country of Residence"];
        log.debug("Country Catched : ",country);
        if(country === null){
            log.warn("Trigram not found. Return NOT IN LIST");
            country="NOT IN LIST";
        }
